...
async function scheduleHtmlProvider(
  ...
  let ht = ''
  try {
    dom = window.frames['frmbody'].frames['frmDesk']
    if (dom.document.getElementById('xnxq') && !dom.frames['frame_1']) {
      let xnxq = dom.document.getElementById('xnxq').value.split('-')
      let html = await request(
        'post',
        'utf8',
        '/xjdxjw/frame/desk/showLessonScheduleInfosV14.action?xn=' +
          xnxq[0] +
          '&xq=' +
          xnxq[1]
      )
      ...
      dom.frames['frame_1'] &&
      dom.frames['frame_1'].contentDocument.getElementById('cxfs_ewb') !=
        undefined
    ) {
      ...
      let info = JSON.parse(
        await request('get', 'gbk', '/xjdxjw/jw/common/showYearTerm.action')
      )
      let courseTable = await request(
        'get',
        'gbk',
        '/xjdxjw/student/wsxk.xskcb10319.jsp?params=' +
          base64Encode(
    ...
    try {
      console.log('err')
      let info = JSON.parse(
        await request('post', 'gbk', '/xjdxjw/jw/common/showYearTerm.action')
      )
      ht = await request(
        'get',
        'gbk',
